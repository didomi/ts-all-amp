...
function updateInputs() {
  ...
  var cBodyObject = JSON.parse(
    decodeURIComponent(
      atob(new URL(document.location.href).searchParams.get("cbody")),
    ),
  );

  Array.from(document.querySelectorAll("[data-config], [data-acb]")).forEach(
    (input) => {
      if (
        input.hasAttribute("data-acb") &&
        ...
        if (input.type == "text")
          input.value = _.get(cBodyObject, propertyPath);
        if (input.type == "checkbox")
          input.checked = _.get(cBodyObject, propertyPath);
      }
    },
  );
  ...
  Array.from(document.querySelectorAll("textarea")).forEach((textArea) => {
    ...
    if (
      textArea.getAttribute("data-format") == "json" &&
      new URL(document.location.href).searchParams.get(urlParam)
    ) {
      prettyPrint(
        textArea,
        atob(new URL(document.location.href).searchParams.get(urlParam)),
      );
      return;
    }
    if (
...
function updateUrl() {
  ...
  if (isJSONvalid(document.querySelector('[data-format="json"]').value)) {
    ...
    params += `&${document
      ...
      .getAttribute("data-qp")}=${btoa(jsonStr)}`;
  }
...
function writeConfig(input, textarea) {
  ...
  if (input.getAttribute("type") == "checkbox") {
    ...
    var inputConfig = JSON.parse(
      input.getAttribute("data-config").replace("@@", `${value}`),
    );
  } else {
    var inputConfig = JSON.parse(
      input.getAttribute("data-config").replace("@@", `"${input.value}"`),
    );
  }

  prettyPrint(
    textarea,
    JSON.stringify(removeEmptyObjects(_.merge(textareaJSON, inputConfig))),
  );
}
